...
const Destination = ({ destinationName }: DestinationProps) => {
  ...
  const {
    register: registerEnq,
    control: controlEnq,
    handleSubmit: handleSubmitEnq,
    formState: { errors },
  } = useForm<FormValues>({ defaultValues: { travelDate: null } });
  ...
  const filteredPlans = Plans.filter((plan) => {
    const nights = plan.duration.split(",").map((n) => n.trim());
    const destinations = plan.destinations.split(",").map((d) => d.trim());
    ...
    const nightsMatch =
      !selectedNights.length || selectedNights.some((n) => nights.includes(n));
    ...
    const priceMatch =
      ...
      selectedPrice.some((range) => {
        ...
        return maxPrice >= selectedMin && minPrice <= selectedMax;
      });

    return nightsMatch && destinationMatch && categoryMatch && priceMatch;
  });
    ...
    <div className="w-full overflow-hidden">
      ...
      <div className="relative h-[55vh] flex flex-col justify-center items-center overflow-hidden">
        ...
        <a
          ...
          className="mt-10 px-5 py-2.5 btn-primary rounded-[5px] text-white cursor-pointer"
        >
          Request Callback
        </a>
        ...
        <div className="mx-auto max-w-350 px-5">
          <div className="flex gap-10 lg:h-[calc(100vh-120px)]">
            ...
            <aside className="hidden lg:block w-[20%] border-t-4 border-[#3fa72a] h-full">
              <div
                className="sticky top-0 h-fit max-h-[calc(100vh-100px)] overflow-y-auto shadow-2xl"
            ...
            {/* CARDS – SCROLLABLE */}
            <main
              className="w-full lg:w-[80%] h-full lg:overflow-y-auto pr-2 bg-gray-100 flex flex-col gap-7"
                  ...
                  <div className=" w-full flex flex-col gap-y-4 p-6">
                    <div>
                      ...
                      <p className="text-[16px] text-black">
                        We would like to understand your plans and come up with
                        the right strategic advice for best tour package for
                        you.
                      </p>
                    </div>
      ...
      <Modal isOpen={openDialog} setIsOpen={setOpenDialog}>
        <EnquiryForm
          image="https://www.holidays2cherish.com/public/upload/holidays2cherish/images/enquiry_holidays2cherish.jpg"
          title="Haven't found an offer you like?"
        />
      </Modal>
    </div>
  );
